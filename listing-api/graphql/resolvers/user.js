...
module.exports = {
  ...
  Mutation: {
      ...
      try {
        if (args.userInput.facebookId) {
          ...
          if (existingfacebookId) {
            throw new Error("Facebook account is already registered. Please Login");
          }
        }
    ...
    updateUser: async (_, args, { req, res }) => {
      ...
      if (!(await checkPhoneAlreadyUsed(req.userId, args.userInput.phone))) {
        try {
          user.name = args.userInput.name;
          user.phone = args.userInput.phone;
          user.description = args.userInput.description;
